...
enum { SchemeNorm, SchemeNormTags, SchemeSel, SchemeHighlight }; /* color schemes */
...
/* variables */
static const char autostartblocksh[] = "gde-autostart-blocking";
static const char autostartsh[] = "gde-autostart";
...
/* Create pixmap array based on tags from config */
static Pixmap pixmap[LENGTH(tags)];
static Pixmap clipmask[LENGTH(tags)];
static unsigned int pixw[LENGTH(tags)], pixh[LENGTH(tags)];
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		if (urg & 1 << i)
			...
			drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNormTags]);
	...
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeNormTags]);
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0);

	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeSel : SchemeNormTags]);
...
getstatusbarpid()
{
	...
	if (statuspid > 0) {
		...
		if ((fp = fopen(buf, "r"))) {
			...
			if(ret == NULL) {
				fprintf(stderr, "fgets: Failed to get status bar pid: %d\n", errno);
			}
			while ((c = strchr(str, '/')))
				str = c + 1;
			fclose(fp);
...
runautostart(void)
{
	...
	if (! (stat(pathpfx, &sb) == 0 && S_ISDIR(sb.st_mode))) {
		 ...
		 * so we try default scripts instead
		 */
		char *pathpfx_new = realloc(pathpfx, strlen(bindir) + 1);
		if(pathpfx_new == NULL) {
			free(pathpfx);
			return;
		}
	...
	if (access(path, X_OK) == 0) {
		ret = system(path);
		if (ret < 0)
			fprintf(stderr, "autostartblocking startup script failed\n");
	}
...
getparentprocess(pid_t p)
{
	...
	ret = fscanf(f, "%*u %*s %*c %u", &v);
	if (ret < 0) {
		fprintf(stderr, "Failed to read parent process\n");
	}
	fclose(f);
#endif /* __linux__*/
